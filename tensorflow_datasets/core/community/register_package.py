...
class PackageRegister(register_base.BaseRegister):
  ...
  def builder_cls(
    ...
    import pathlib
    for f in pathlib.Path(cache.cache_path()).rglob('*'):
      print(f)
...
def _compute_dir_hash(path: utils.ReadOnlyPath) -> str:
  ...
  all_checksums = [
      checksums.compute_url_info(file).checksum
...
community_register = PackageRegister(
    path=utils.gcs_path() / 'community-datasets-list.jsonl',
)
