...
class C4(tfds.core.BeamBasedBuilder):
  ...
  def _get_page_content(self, pipeline, file_paths, dl_manager):
    ...
    wet_file_paths = (
        ...
        beam.Filter(lambda p: p not in _KNOWN_CORRUPT_WET_FILES) | beam.Map(
            download_wet_file,
    ...
    # Output: url, text
    page_content = (
        wet_file_paths | beam.FlatMap(c4_utils.split_wet_file) |
    ...
    # Output: url, text
    page_content = (
        page_content | "normalize_url" >> beam.Map(c4_utils.normalize_url) |
        "group_url" >> beam.GroupByKey() | beam.Map(c4_utils.dedupe_urls)
    )
    ...
    if self.builder_config.webtextlike:
      webtextlike_urls = (
          pipeline | "read_webtextlike_urls" >> beam.io.ReadFromText(
              os.path.join(
  ...
  def _build_pcollection(
      self, unused_pipeline, split, page_content, split_filter_fn
  ):
    beam = tfds.core.lazy_imports.apache_beam
    ...
    return (
        page_content | beam.Filter(split_filter_fn) | beam.Map(_emit_examples)
    )
